...
# Define service schemas (these can mirror what's in your services.yaml if you wish)
...
async def async_setup_kill_stream_services(hass: HomeAssistant, entry, api: TautulliAPI) -> None:
    """Register kill stream services for Tautulli Active Streams."""
    _LOGGER.debug("Starting async_setup_kill_stream_services")
    
    # Register kill all streams service
    try:
    ...
    # Register kill user stream service
    try:
        hass.services.async_register(
            DOMAIN,
            "kill_user_stream",
            handle_kill_user_stream,
...
async def handle_kill_all_streams(call: ServiceCall) -> None:
    """Handle kill all streams service call."""
    _LOGGER.debug("handle_kill_all_streams called with data: %s", call.data)
    message = call.data.get("message")
    # Assuming your coordinator is stored in hass.data[DOMAIN][entry.entry_id]
    coordinator = hass.data[DOMAIN].get(call.context.get("entry_id"))
...
async def handle_kill_user_stream(call: ServiceCall) -> None:
    """Handle kill user stream service call."""
    _LOGGER.debug("handle_kill_user_stream called with data: %s", call.data)
    ...
    if not coordinator or "sessions" not in coordinator.data:
        ...
        return
    sessions = coordinator.data.get("sessions", [])
    matched_sessions = [s for s in sessions if target_user in (
